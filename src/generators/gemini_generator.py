...
class GeminiArticleGenerator:
    ...
    def generate_article(
        ...
        Returns:
            Dict: 生成結果 {"success": bool, "content": str, "error": str}
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if generated_content and len(generated_content.strip()) > 100:
                    logger.info("記事生成成功")
                    return {"success": True, "content": generated_content, "error": None}
                else:
                    logger.warning("生成された記事が短すぎます")
                    if attempt == max_retries - 1:
                        return {"success": False, "content": "", "error": "生成された記事が短すぎます。"}
                ...
                if "quota" in error_msg.lower() or "rate" in error_msg.lower():
                    if attempt < max_retries - 1:
                        ...
                        return {"success": False, "content": "", "error": "APIのレート制限に達しました。しばらく待ってから再試行してください。"}

                # その他のエラー
                if attempt < max_retries - 1:
                    time.sleep(1)
                else:
                    return {"success": False, "content": "", "error": f"Gemini APIエラーが発生しました: {error_msg}"}

        return {"success": False, "content": "", "error": "記事生成に失敗しました。"}
