...
from mock_generator import MockArticleGenerator, MockGeminiArticleGenerator
...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        <div class="test-mode-banner">
            <strong>🧪 テストモードで実行中（モック機能使用）</strong>
        </div>
        """, unsafe_allow_html=True)
        ...
        ai_provider = st.selectbox(
            "AIプロバイダー",
            ["OpenAI", "Gemini"],
            help="使用するAIサービスを選択してください（テストモード）"
        )
        ...
        if ai_provider == "OpenAI":
            ...
            api_key = st.text_input(
                "OpenAI APIキー",
                type="password",
                value="test_key_for_demo",
                help="テスト用: test_key_for_demo"
            )
            ...
            model = st.selectbox(
                ...
                index=0,
                help="使用するOpenAIモデルを選択してください（テストモード）"
            )
            ...
            api_key = st.text_input(
                "Gemini APIキー",
                type="password",
                value="test_gemini_key_for_demo",
                help="テスト用: test_gemini_key_for_demo"
            )
            ...
            model = st.selectbox(
                ...
                index=0,
                help="使用するGeminiモデルを選択してください（テストモード）"
            )
        ...
        if st.button("🔍 APIキー検証"):
            if api_key:
                try:
                    if ai_provider == "OpenAI":
                        generator = MockArticleGenerator(api_key, model)
                    else:
                        generator = MockGeminiArticleGenerator(api_key, model)
        ...
        if st.session_state.api_validated:
            st.success(f"✅ {ai_provider} APIキーが有効です（モックモード）")
...
def create_article_generation_tab():
    ...
    st.markdown("""
    <div class="test-mode-banner">
        🔬 <strong>テストモード</strong>: 実際のOpenAI APIやGemini APIは使用せず、モック機能でデモンストレーションを行います。
    </div>
    """, unsafe_allow_html=True)
